...
# purposes that not every python program will
...
def sort_counts(counts):
    ...
    return sorted_tuples    # sorted_tuples[0] == most frequent
...
def write_frequencies(frequencies, path):
    ...
    for l in frequencies:
        sum_words = sum_words + l[1]        # calculate total sum of tokens for frequcency below

    with open(path, "w") as f:
         for l in frequencies:
             rank = rank + 1
             f.write(f"{rank},{l[0]},{l[1]},{l[1]/sum_words}\n")     # f"..." to make formatting easier
